...
| 4 | OpenAI async compatibility error mapping | `COMPATIBILITY` marker and `LegacyOpenAiV1` branch in `src/services/voice-service.ts`, the policy enum, composition selection, and policy-specific tests | OpenAI async rollback support explicitly retired, or a breaking error-contract rollout approved |
