...
2. Session and quota limits are checked before forwarding each aligned frame. The effective cap is `min(REALTIME_SESSION_MAX_SECONDS, remainingDailySeconds)`; a tie reports `quota_limit`, otherwise the bound that produced the minimum fixes the completion reason and moves to `finishing`.
...
- Explicit EU Soniox WebSocket URL is constructed from validated configuration and passed to the SDK.
