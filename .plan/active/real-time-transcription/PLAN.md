...
6. Audio, transcripts, provisional text, JWTs, provider keys, raw project IDs, and raw user IDs are absent from persistence and logs. Project glossary terms are persisted only in the account- and project-scoped glossary collection, never logged, and purged with account deletion. Soniox content uses the approved US project and endpoints.
