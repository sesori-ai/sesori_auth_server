...
| Private app feedback       | `src/routes/feedback.ts` + `src/services/feedback-service.ts` + `src/repositories/feedback-repo.ts` + `src/models/feedback.ts` | `POST /feedback` stores one document per submission. Never log the message text. Account deletion is manual and must also `deleteMany` the user's `feedback` documents (README, Feedback) |
