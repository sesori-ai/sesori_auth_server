...
- **Any store that parks an HTTP long poll must implement `ShutdownRequestWaiters` and be registered in `requestWaiters`.** Fastify close waits for in-flight requests, so shutdown releases parked polls as an early timeout, stops admitting new waits, and drains repository reads detached by the release before MongoDB closes.
...
`ActivationReminderService` is disabled by default. Its delivery order is due query, immediate eligibility recheck, FCM send, then conditional sent-marker write. Genuine zero-device results are marked complete; FCM-unavailable, thrown sends, and zero-success results with retryable token failures remain eligible. On shutdown, `dispose()` stops queued candidates and waits up to `ACTIVATION_REMINDER_DISPOSE_TIMEOUT_MS` (15 seconds) for the current candidate through its marker write. If that budget expires, the service force-fences itself and rejects with `ActivationReminderDrainFenced`; the coordinator treats this as a drain failure, leaves MongoDB open, and exits 1 at the T+22 hard deadline rather than closing the client under a detached callback. A late FCM result cannot start stale-token cleanup or a marker after fencing. Later backfill behavior must follow the staged acceptance criteria in the plan.
