...
│   ├── account-rate-limit-key.ts      # Per-route limiter key: verified userId, else client IP
...
| Private app feedback       | `src/routes/feedback.ts` + `src/services/feedback-service.ts` + `src/repositories/feedback-repo.ts` + `src/models/feedback.ts` | `POST /feedback` stores one document per submission; `DELETE /feedback` clears the account's feedback for account deletion. Never log the message text |
...
- **Rate-limit counters are in-process only.** `@fastify/rate-limit` keeps every allowance — the global one and the per-route settings-write and feedback-write ones — in a process-local LRU, so N instances grant N times the intended allowance. The route store also defaults to 5000 keys, and an evicted key starts a fresh window, so a limit is a bound on sustained abuse rather than a hard guarantee once distinct keys exceed that. Raise `cache` or move to a shared store before scaling out or before treating any limit as a correctness control. Route limits are compile-time constants (`SETTINGS_WRITE_MAX_PER_MINUTE`, `FEEDBACK_WRITE_MAX_PER_HOUR`, `REALTIME_START_MAX_PER_MINUTE`, and the voice/session equivalents), so retuning one is a deploy, not an env flip.
