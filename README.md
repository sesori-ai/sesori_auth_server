...
The allowance is keyed on the access token's `userId` claim, not the token string, so refreshing does not hand out a new allowance. That claim is read without verifying the signature, which is safe for keying alone: a forged token still fails authentication and writes nothing, so it can only split its own buckets while burning 401s against the global allowance.
