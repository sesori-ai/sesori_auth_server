...
const productAnalyticsExportConfigSchema = z.object({
  ...
  PRODUCT_ANALYTICS_INTERNAL_EXCLUSION_VIEW: z
    .string()
    .regex(/^[a-z][a-z0-9-]{4,28}[a-z0-9]\.[A-Za-z_][A-Za-z0-9_]{0,1023}\.[A-Za-z_][A-Za-z0-9_]{0,1023}$/),
...
export function loadProductAnalyticsExportConfig(input: { env?: NodeJS.ProcessEnv }): ProductAnalyticsExportConfig {
  ...
  if (!result.success) {
    throw productAnalyticsConfigurationError({ description: "product analytics export", error: result.error });
  }
  return result.data;
}
