...
describe("createAuthMiddleware", () => {
  ...
  it("rejects a refresh token presented as an access token", async () => {
    ...
    const token = tokenService.signRefreshToken({ userId, tokenVersion: 0 });
  ...
  it("does not let a bypassed request mutate the shared development user", async () => {
    const requireAuth = createAuthMiddleware(buildTokenService(), { devBypassEnabled: true });
    const first = buildRequest();
    const second = buildRequest();

    await requireAuth(first, reply);
    first.user!.userId = "mutated";
    await requireAuth(second, reply);

    assert.notEqual(second.user?.userId, "mutated");
  });
});
